...
$(function(){
  ...
  var score;
    ...
    function updateStats(){
      $('#stats').html('<div class="padded"><h2>Цифры: <span>'+'<b>'+get('flip_won')+'</b><i>Выиграно</i>'+
        '<b>'+get('flip_lost')+'</b><i>Проиграно</i>'+
        '<b>'+get('flip_abandoned')+'</b><i>Прекращено</i></span></h2>'+
        '<ul><li><b>Лучший Простой:</b> <span>'+toTime( get('flip_casual') )+'</span></li>'+
        '<li><b>Лучший средний:</b> <span>'+toTime( get('flip_medium') )+'</span></li>'+
        '<li><b>Лучший сложный:</b> <span>'+toTime( get('flip_hard') )+'</span></li></ul>'+
        '<ul><li><b>Перевернуто карточек:</b> <span>'+parseInt( ( parseInt(get('flip_matched'))
             + parseInt(get('flip_wrong')) ) * 2)+'</span></li>'+
        '<li><b>Совпадающих карточек:</b> <span>'+get('flip_matched')+'</span></li>'+
        '<li><b>Неверных карточек:</b> <span>'+get('flip_wrong')+'</span></li></ul></div>');
    };
    ...
    $('.play').on('click', function(){
      ...
      $('.logo').fadeOut(250, function(){
        ...
        for(i = 0; i < shu.length; i++){
          ...
          if(code == 30) code = 10;
          if(code == 31) code = 21;
          $(`<div class="card" style="width:${cardSize}%;height:${cardSize}%;"><div class="flipper"><div class="f"></div><div class="b" data-f="&#xf0${code};"></div></div></div>`).appendTo('#g');
        }
              ...
              setTimeout(function(){
                ...
                if( thisCard.length > 1 ) {
                  ...
                  if( !$('#g .card').length ){
                    ...
                    score = (get('flip_'+difficulty)/10);
                    sendToServer (score);
                    $("#result").html(JSON.stringify(score));
